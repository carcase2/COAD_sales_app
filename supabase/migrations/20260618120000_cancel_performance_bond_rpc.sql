-- 이행증권 취소 RPC — 앱에서 RLS(app.current_user_*) 없이도 status를 cancelled로 갱신.
-- 웹 PATCH /api/performance-bonds/[id] 권한 규칙과 동일.
...
begin
  ...
  select coalesce(array_agg(distinct ug.group_id), array[]::uuid[])
    into v_group_ids
    from user_groups ug
   where ug.user_id = v_user.id;

  if coalesce(array_length(v_group_ids, 1), 0) = 0 and v_user.group_id is not null then
    v_group_ids := array[v_user.group_id];
  end if;
